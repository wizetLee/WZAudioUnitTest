...
//    }
    ...
    {//配置音频流格式input
        ...
        audioStreamFormat.mFormatFlags        = kAudioFormatFlagsNativeEndian | kAudioFormatFlagIsPacked | kAudioFormatFlagIsFloat;
        audioStreamFormat.mFramesPerPacket    = 1;//每个数据包多少帧
        audioStreamFormat.mChannelsPerFrame   = 1;//1单声道，2立体声
        unsigned wordsize = 4;
        audioStreamFormat.mBitsPerChannel     = wordsize * 8;//语音每采样点占用位数
...
//        audioStreamFormat.mBytesPerPacket     = audioStreamFormat.mBytesPerFrame * audioStreamFormat.mFramesPerPacket;//每个数据包的bytes总数，每帧的bytes数＊每个数据包的帧数
        ...
        BOOL inIsInterleaved = true;
        if (inIsInterleaved) {//立体音效
            audioStreamFormat.mBytesPerFrame = audioStreamFormat.mBytesPerPacket = wordsize * audioStreamFormat.mChannelsPerFrame;
        } else {
            audioStreamFormat.mFormatFlags |= kAudioFormatFlagIsNonInterleaved;
            audioStreamFormat.mBytesPerFrame = audioStreamFormat.mBytesPerPacket = wordsize;
        }
    ...
    {//output render call    input scope, element0 中设置render call得到音频buffer（此时得到的数据还没有输出）
